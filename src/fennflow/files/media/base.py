...
import mimetypes
from abc import ABC
from typing import Literal, Any, Self, ClassVar
import os
...
class BaseContent(BaseModel, ABC):
    ...
    filename_factory: ClassVar["FileDataExtNamingFactoryProtocol"] = get_determined_filename_by_obj
    ...
    def validate_filename_and_media_type(self) -> Self:
        if self.media_type and self.filename:
            ...
            guessed_media_type = mimetypes.guess_file_type(path=self.filename, strict=False,)[0]
    ...
    def extension(self) -> str:
        if self.filename:
            ...
            logger.debug(f"Filename is not specified. Trying to determine extension from {self.media_type=}")
            extension = mimetypes.guess_extension(self.media_type, strict=False,)

            if extension is None:
                raise ExtensionCannotBeGuessed(f"Cannot guess extension for {self.media_type=}. Please, specify filename explicitly.")
    ...
    def get_metadata(self, exclude: set = None) -> dict:
        exclude = exclude or set()
        exclude.update({"data", "media_type"})
