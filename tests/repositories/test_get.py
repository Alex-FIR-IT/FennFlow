...
async def test_put_and_get(uow_cls, text_files):
    async with uow_cls() as uow:
        ...
        assert len(result) == 1
        assert result[0].content.data == text_files[0].data

        result2 = await uow.user_files.get(text_files[1].filename)

        assert len(result2) == 1
        assert result2[0].content.data == text_files[1].data
...
async def test_get_several_files_at_once(uow_cls, text_files):
    async with uow_cls() as uow:
        ...
        assert len(result) == 2
        assert result[0].content.data == text_files[0].data
        assert result[1].content.data == text_files[1].data
        ...
        assert len(result) == 2
        assert result[0].content.data == text_files[1].data
        assert result[1].content.data == text_files[0].data
